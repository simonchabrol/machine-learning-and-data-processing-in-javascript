    ...
    for (var b = 0; b < DataSet.length; b++) {
        ...
        if (Array.isArray(TestedValues) === false) {
            if (typeof TestedValues === 'string' && isNaN(TestedValues)) {
                ...
                console.log(TestBloc[Fields[a]])
                TestedValues = TestedValues.trim().split(' ')
            } else if (typeof TestedValues === 'string' && !isNaN(TestedValues)) {
                TestBloc[Fields[a]] = [parseFloat(TestedValues)]
                TestedValues = [parseFloat(TestedValues)]
            } else {
    ...
    if (TypesName == 'Input') {
        ...
        if (ValuesName == 'number') {
            ...
            for (var f = 0; f < DataSet.length; f++) {
                Bloc = DataSet[f]
                var ValuesNumbers = Bloc[Fields[a]]
                Numbers = Numbers.concat(ValuesNumbers)
            }
            var Maximum = Math.max(...Numbers)
            var Divisor = 1
            for (var g = 0; g < parseInt(Maximum).toString().length; g++) {
                Divisor = Divisor.toString().concat(0)
            }
            for (var h = 0; h < DataSet.length; h++) {
                Bloc = DataSet[h]
                ValuesNumbers = Bloc[Fields[a]]
                Input[h] = Input[h].concat(Math.round((ValuesNumbers / Divisor) * 100) / 100)
                //Input[h] = Input[h].concat(ValuesNumbers)
            }
        }
    }
    if (TypesName == 'Output') {
...
/*
 [
   ...
   Input: [[1,1,0,0,0,0,0,0,0.15],[0,0,1,1,0,0,0,0,0.17],[0,0,1,0,1,1,1,1,0.08]]
   Output: [0,0,1]
  */
